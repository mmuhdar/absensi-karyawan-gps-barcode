...
class AttendanceController extends Controller
{
    ...
    //     // Adjust the logic for selecting the dates (weekly/monthly/daily)
    //     if ($request->date) {
    //         $dates = [$carbon->parse($request->date)->settings(['formatFunction' => 'translatedFormat'])];
    //     } else if ($request->week) {
    //         // Adjust the start and end of the week to be Monday to Saturday
    //         $start = $carbon->parse($request->week)->startOfWeek()->addDay(); // start on Monday
    ...
    //     // Fetch employees with the selected filters (division, job title)
    //     $employees = User::where('group', 'user')
    //         ->when($request->division, fn(Builder $q) => $q->where('division_id', $request->division))
    //         ->when($request->jobTitle, fn(Builder $q) => $q->where('job_title_id', $request->jobTitle))
    //         ->get()
    //         ->map(function ($user) use ($request, $carbon) {
    ...
    //                     "attendance-$user->id-$request->date",
    //                     now()->addDay(),
    //                     function () use ($user, $request, $carbon) {
    //                         $date = Carbon::parse($request->date);
    ...
    //                     now()->addDay(),
    //                     function () use ($user, $request, $carbon) {
    //                         $start = Carbon::parse($request->week)->startOfWeek()->addDay(); // Monday
    //                         $end = $start->copy()->addDays(5); // Saturday

    //                         // Fetch attendance for the entire week (Monday to Saturday)
    //                         $attendances = Attendance::where('user_id', $user->id)
    //                             ->whereBetween('date', [$start->toDateString(), $end->toDateString()])
    //                             ->get(['id', 'status', 'date']);
    ...
    //             // Assign the attendance records to the user
    //             $user->attendances = $attendances;
    //             return $user;
    //         });
